...
// 🚨 Ejercicio: Reembolsar los pagos de alerta temprana de fraude
// Abre el archivo del servidor y navega al mensaje del ejercicio de alerta temprana de fraude. Copia el mensaje.
// Navega a tu implementación del endpoint /webhook. Dentro del endpoint, antes de la verificación del tipo de evento, pega el mensaje del ejercicio.

// Usando los comentarios como guía:
// - Escucha eventos tipo 'charge.dispute.created' o 'review.opened' para detectar alertas tempranas de fraude.
// - Si se detecta una alerta:
//     1. Obtén el ID del cargo (charge id) del evento
//     2. Llama a `stripe.refunds.create({ charge: 'ch_123' })`
//     3. Imprime en consola que se procesó un reembolso por alerta de fraude

// Para probarlo:
// 1. Inicia la aplicación Fedorable con `npm run dev`
// 2. En otra terminal, ejecuta:
//    stripe listen --forward-to localhost:3000/webhook
// 3. Navega a la app, llena un carrito y haz clic en pagar
// 4. Usa una tarjeta de prueba de Stripe que dispare alerta temprana (por ejemplo: 4000 0027 6000 3184)
// 5. Observa en consola que se generó un reembolso automático

// Detén la aplicación y el escucha de webhook con Ctrl+C
// app.post('/webhook', express.raw({type: 'application/json'}), (req, res) => {
//   const sig = req.headers['stripe-signature'];
//   let event;
//   try {
